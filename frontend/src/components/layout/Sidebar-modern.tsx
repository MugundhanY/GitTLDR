...
export default function Sidebar({ selectedRepository }: SidebarProps) {
  ...
  const { stats, isLoading: statsLoading } = useRepositoryStats(selectedRepository?.id);
  ...
  const secondaryItems = [
    ...
    <div className={`flex h-full flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 ${
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        {!isCollapsed && (
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="w-full flex items-center space-x-2 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors"
            >              {selectedRepository ? (
                <>
                  {/* Repository Avatar */}
        ...
        {isCollapsed && selectedRepository && (
