...
class FileProcessor:
    ...
    async def process_full_repository(self, task_data: Dict[str, Any], task_logger) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._process_repository_commits(temp_dir, repo_id, task_logger)
    ...
    async def _update_repository_completion(self, repo_id: str, summary: str, stats: Dict):
        ...
        try:            # Store completion data in Redis for node-worker to update database
            ...
            # Don't fail the entire task for status update issues
    async def _process_repository_commits(self, repo_path: str, repo_id: str, task_logger):
        """Fetch and process repository commits."""
        from services.redis_client import redis_client
        import subprocess
        import json
        ...
        try:
            ...
            # Get last 50 commits using git log
            git_log_cmd = [
                "git", "log", "--max-count=50", "--pretty=format:%H|%s|%an|%ae|%ad|%P",
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if '|' in line and len(line.split('|')) >= 6:
                    ...
                    # Parse commit line: hash|subject|author|email|date|parents
                    parts = line.split('|')
                    current_commit = {
                        'sha': parts[0],
                        'message': parts[1],
            ...
            for commit in commits_to_queue_pending:
                try:
                    # Create commit result for node-worker processing (pending status)
                    commit_result = {
                        "type": "commit_summary",
                        "repositoryId": repo_id,
                        "commitSha": commit['sha'],
